...
public class BookingRequestDao implements BookingRequestDaoImpl {
    ...
    public List bookingRequest(BookingRequestBean brb) {
        ...
        try {
            ...
            PreparedStatement pst1 = connection.prepareStatement("select UserDetails.Name,VegetableDetails.VegetableName,Status.BookingQuantity,Status.Date,Status.Status from Status INNER JOIN UserDetails ON Status.BookedTo= UserDetails.UserId INNER JOIN VegetableDetails ON Status.VegetableId=VegetableDetails.VegetableId where Status.UserId=?");
